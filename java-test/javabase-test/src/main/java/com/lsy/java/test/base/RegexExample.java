...
//        matchLookAt(); //matches 和 lookingAt 方法

//        matchReplace();//replaceFirst 替换首次匹配，replaceAll 替换所有匹配。

        matchAppend();//appendReplacement用于文本替换 和 appendTail 用于尾部追加

    }

    static void matchAppend(){
        String REGEX = "a*b";
...
//        INPUT = m.replaceAll(REPLACE);
    ...
    static void isMatch(){
        String content = "I am 123 " +
                "from cdfdf.com.";

        String pattern = ".*cdfdf.*";

        boolean isMatch = Pattern.matches(pattern, content);
        System.out.println("字符串中是否包含了 'cdfdf' 子字符串? " + isMatch);
    }

    static void matchGet(){
        // 按指定模式在字符串查找
 ...
 *
 ...
 例如，"\x41"匹配"A"。"\x041"与"\x04"&"1"等效。允许在正则表达式中使用 ASCII 代码。
 ...
 则 n 是反向引用，后面跟有字符 m。如果两种前面的情况都不存在，则 \nm 匹配八进制值 nm，其中 n 和 m 是八进制数字 (0-7)。
