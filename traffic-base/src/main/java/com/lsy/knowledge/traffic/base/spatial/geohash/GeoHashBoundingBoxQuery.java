...
public class GeoHashBoundingBoxQuery implements GeoHashQuery, Serializable {
	...
	//modify search geohash with the given fittingBits and fix the bug
	public GeoHashBoundingBoxQuery(BoundingBox bbox, int fittingBits) {
		WGS84Point center = bbox.getCenterPoint();
		GeoHash centerHash = GeoHash.withCharacterPrecision(center.getLatitude(), center.getLongitude(), 6);

		Queue<GeoHash> queue = new LinkedList<GeoHash>();
		queue.add(centerHash);
		while(!queue.isEmpty()){
			centerHash = queue.poll();
			if (hashFits(centerHash, bbox) || centerHash.getBoundingBox().intersects(bbox)){
